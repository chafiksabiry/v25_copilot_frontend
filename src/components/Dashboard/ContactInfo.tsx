...
export function ContactInfo() {
  ...
  return (
    <div className="relative group">
      <div className="glass-card rounded-2xl shadow-2xl px-10 py-8 flex items-center justify-between mt-6 mb-8 border border-white/5 hover:border-white/10 transition-all duration-700 relative overflow-hidden">
        ...
        <div className="flex items-center space-x-6 relative z-10">
          {leadLoading ? (
            ...
            <>
              <div className="w-20 h-20 rounded-3xl bg-gradient-harx flex items-center justify-center p-1.5 shadow-2xl shadow-harx-500/20 transform group-hover:rotate-3 transition-transform duration-700">
                <div className="w-full h-full rounded-[20px] overflow-hidden bg-slate-900 flex items-center justify-center border border-white/10">
                    {contact.avatar ? (
                    <img src={contact.avatar} alt={contact.name} className="w-full h-full object-cover" />
                    ) : (
                    <span className="text-white font-black text-2xl tracking-tighter">{contact.name.split(' ').map(n => n[0]).join('')}</span>
                    )}
                </div>
              </div>
              <div>
                <div className="flex items-center space-x-3 mb-2">
                  <span className="text-2xl font-black text-white tracking-tight">{contact.name}</span>
                  <span className="bg-emerald-500/10 text-emerald-400 text-[9px] px-3 py-1 rounded-full font-black border border-emerald-500/20 uppercase tracking-[0.2em]">Verified Priority</span>
                  {gig && (
                    <span className="bg-white/5 text-slate-400 text-[9px] px-3 py-1 rounded-full border border-white/10 flex items-center font-black uppercase tracking-widest shadow-inner">
                      <Briefcase className="w-3.5 h-3.5 mr-2 text-harx-500" />
                      {gig.title}
                    </span>
                  )}
                </div>
                <div className="flex items-center space-x-3 text-slate-500 text-[10px] font-black uppercase tracking-widest group-hover:text-slate-300 transition-colors">
        ...
        <div className="flex-1 flex flex-col items-center relative z-10">
          {callStatus === 'active' ? (
            <button
              onClick={endCall}
              className="w-64 flex items-center justify-center space-x-3 px-6 py-4 rounded-2xl font-black text-sm uppercase tracking-[0.25em] transition-all duration-500 shadow-2xl bg-rose-600 hover:bg-rose-700 text-white hover:-translate-y-1 active:scale-95 border border-rose-500/50 shadow-rose-600/20 group/call"
            >
              <div className="p-2 bg-white/20 rounded-xl group-hover/call:rotate-12 transition-transform">
            ...
            <button
              ...
              className={`w-64 flex items-center justify-center space-x-3 px-6 py-4 rounded-2xl font-black text-sm uppercase tracking-[0.25em] transition-all duration-500 shadow-2xl hover:-translate-y-1 active:scale-95 group/call
            ...
            >
              <div className={`p-2 rounded-xl transition-all duration-500 ${isCallLoading || callStatus === 'initiating' ? 'bg-slate-700' : 'bg-white/20 group-hover/call:rotate-12 transform shadow-inner'}`}>
              ...
              <span>{isCallLoading || callStatus === 'initiating' ? 'Syncing...' : 'Initiate Call'}</span>
            </button>
          )}

          <div className="flex items-center space-x-3 text-slate-500 text-[10px] font-black uppercase tracking-[0.2em] mt-3 opacity-40 group-hover:opacity-100 transition-opacity duration-700">
            ...
            <span className="text-emerald-500/60 lowercase italic tracking-tight font-medium">Ready for stream</span>
          </div>
        </div >
        {/* Actions à droite */}
        <div className="flex items-center space-x-4 relative z-10">
          <button className="bg-white/5 border border-white/5 text-slate-500 p-4 rounded-2xl cursor-not-allowed group-hover:bg-white/10 group-hover:border-white/10 transition-all duration-500 shadow-inner" title="Schedule Follow-up">
            <Calendar className="w-6 h-6" />
          </button>
          <button className="bg-gradient-harx text-white p-4 rounded-2xl shadow-xl shadow-harx-500/20 hover:-translate-y-1 active:scale-95 transition-all duration-300 border border-white/20">
            <Phone className="w-6 h-6 fill-white/20" />
          </button>
          <button className="bg-white/5 border border-white/5 text-slate-500 p-4 rounded-2xl cursor-not-allowed group-hover:bg-white/10 group-hover:border-white/10 transition-all duration-500 shadow-inner" title="Send Briefing">
